...
and configure the Universal Messaging realm server and also run the runUMTool
to connect/create/delete/get the Universal Messaging realm server assets
...
This package makes the following assumptions:
	...
	- You have Universal Messaging 10.4 installed on a 64-bit Linux machine using
...
and runUMTool. The image is also set up to allow trouble-free execution; suitable environment
...
You can see that an image has been created as follows:
    ...
    universalmessaging-server   10.4     5b733a9b987d    39 seconds ago     415 MB
...
build argument.
	...
	docker build --build-arg __instance_name=testserver --tag universalmessaging-server:10.4
...
the container we will have runUMTool configured for admin usages.
...
Turning your new image into a running container will look something like this:
	...
	docker run -d -p 9010:9000 --name umcontainer universalmessaging-server:10.4
...
You can then look for your running container:
    ...
    a15557bccc7c   universalmessaging-server:10.4   "/bin/sh -c umstart.…"     6 seconds ago      Up 5 seconds       0.0.0.0:9010->9000/tcp   umcontainer
...
You can pass the configurations as follows:
	...
	-e MAX_DIRECT_MEM_SIZE=3G -e BASIC_AUTH_ENABLE=Y -e BASIC_AUTH_MANDATORY=Y -p 9020:9000 --name umcontainer universalmessaging-server:10.4

Interacting with the container
==============================
Now you will be able to interact with the realm server running inside the
container. Because ports from the container have been mapped to the host
operating system's networking stack, you can run Universal Messaging clients
outside of Docker and connect to the Universal Messaging realm
server using nsp://<host>:<mapped_port>.
...
without getting into the container.
...
Image management
================
By default, the images you create will reside on the Docker host machine. 

We push the Docker images to DTR (Docker Trusted Registry) after building them.
You can find the UM Docker image at

	https://daerepository03.eur.ad.sag:4443/repositories/universal-messaging/universalmessaging-server/info
...
locally. For example
	...
	docker run --name umcontainer daerepository03.eur.ad.sag:4443/universal-messaging/universalmessaging-server:10.4

will pull 'universalmessaging-server:10.4' from the Docker Trusted Registry and run it, just as if it was built locally.
If you plan to run your containerized realm server across multiple
different machines, a private registry is the way to go to avoid needless
extra setup. It also helps avoid concerns about managing the configuration across
multiple machines; later samples show how to create Docker images that contain
realm servers with a preconfigured state, e.g. initial sets of channels and ACLs.
