...
sed -i "s|\(wrapper.java.additional.4="\""-DDATADIR\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME/data"\""|" $SERVER_COMMON_CONF_FILE
sed -i "s|\(wrapper.java.additional.5="\""-DSERVERDIR\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME"\""|" $SERVER_COMMON_CONF_FILE
sed -i "s|\(wrapper.java.additional.7="\""-Djavax.net.ssl.trustStore\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME/bin/nirvanacacerts.jks"\""|" $SERVER_COMMON_CONF_FILE
sed -i "s|\(wrapper.java.additional.9="\""-Djavax.net.ssl.keyStore\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME/bin/server.jks"\""|" $SERVER_COMMON_CONF_FILE
sed -i "s|\(wrapper.java.additional.14="\""-DLICENCE_DIR\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME"\""|" $SERVER_COMMON_CONF_FILE
sed -i "s|\(wrapper.java.additional.22="\""-DLOGFILE\)=\(.*\)|\1=$UM_HOME/server/$INSTANCE_NAME/data/nirvana.log"\""|" $SERVER_COMMON_CONF_FILE
...
sed -i "s|\(.*\)=-DADAPTER_0=\(.*\)|\1=-DADAPTER_0=nhp://0.0.0.0:9000|" $SERVER_COMMON_CONF_FILE
...
# As we build the UM image, UM is ready to run as container. So updating the parameter that UM will be running as container.
sed -i "s|\(.*\)=-DUM.running.as.container=\(.*\)|\1=-DUM.running.as.container=Y|" $SERVER_COMMON_CONF_FILE 
